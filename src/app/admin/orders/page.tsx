...
export default function OrdersPage() {
  ...
  const handleStatusChange = async (paymentId: string, newStatus: string, note?: string) => {
    try {
      setActionLoading(true);
      const response = await fetch(`/api/admin/orders/${paymentId}`, {
        method: 'PUT',
        headers: {
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-semibold">{selectedPayment.currency} {selectedPayment.amount.toFixed(2)}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-gray-500">{language === 'ar' ? 'طريقة الدفع' : 'Payment Method'}</span>
                  <span>{getPaymentMethodLabel(selectedPayment.paymentMethod || '')}</span>
                </div>
                <div className="flex items-center justify-between">
              ...
              <div className="border-t border-gray-200 pt-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-medium text-gray-900">{selectedPayment.user.name}</div>
                    <div className="text-sm text-gray-500">{selectedPayment.user.email}</div>
                    {selectedPayment.user.phone && (
                      <div className="text-sm text-gray-500">{selectedPayment.user.phone}</div>
                    )}
                  </div>
                </div>
              </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-medium text-gray-900">
                        {selectedPayment.course.title[language]}
                      </div>
                      <div className="text-sm text-gray-500">
                        {selectedPayment.currency} {selectedPayment.course.price.toFixed(2)}
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {selectedPayment.adminNote && (
                <div className="bg-gray-50 p-4 rounded-lg">
                  <div className="flex items-center gap-2 text-gray-600 mb-2">
                    ...
                    <span className="font-medium">{language === 'ar' ? 'ملاحظة الإدارة' : 'Admin Note'}</span>
                  </div>
                  <p className="text-gray-700">{selectedPayment.adminNote}</p>
                </div>
              )}
            </div>
            ...
            {selectedPayment.status === 'PENDING' && (
              <div className="p-6 border-t border-gray-200 flex gap-3">
                ...
                <button
                  onClick={() => handleStatusChange(selectedPayment.id, 'FAILED', adminNote)}
                  disabled={actionLoading}
            ...
            {selectedPayment.status === 'COMPLETED' && (
              <div className="p-6 border-t border-gray-200">
                <button
                  onClick={() => handleStatusChange(selectedPayment.id, 'REFUNDED', adminNote)}
                  disabled={actionLoading}
